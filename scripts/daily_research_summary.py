...
def drive_upload_with_retry(svc, full_path: str, root: str, data: bytes, mime: str,
    ...
    """Upload with exponential backoff retry for transient 5xx errors and network failures.
    Non-5xx errors (auth, quota, etc.) and permanent failures fail immediately."""
    from googleapiclient.errors import HttpError
    import socket, ssl

    http_backoff = [1, 2, 4]
    network_backoff = [5, 10, 15]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not is_transient or attempt >= max_retries - 1:
                ...
                raise
            print(f"   Transient error (HTTP {status}), retrying in {http_backoff[attempt]}s...")
            time.sleep(http_backoff[attempt])
        except transient_exceptions as e:
            if attempt >= max_retries - 1:
                print(f"   Network error ({full_path}, {len(data)} bytes): {type(e).__name__}")
                raise
            wait_secs = network_backoff[attempt]
            print(f"   Transient network error ({type(e).__name__}), retrying in {wait_secs}s...")
